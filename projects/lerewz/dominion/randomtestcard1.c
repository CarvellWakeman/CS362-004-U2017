...
int main () {
	...
	for (i = 0; i < 100; i++){
		...
		for (j = 0; j < 2; j++){
			...
			expected->deckCount[j] = (actual->deckCount[j])-3;
			expected->handCount[j] = (actual->handCount[j])+3;
		}
		...
		// Call smithy card
		int result = cardSmithy(actual, currentPlayer, handPos);
